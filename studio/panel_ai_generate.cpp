...
void provider_combo(const char *label, std::string &value, const char *purpose_a, const char *purpose_b) {
  ...
  if (ImGui::BeginCombo(label, cur ? cur->name : value.c_str())) {
    for (const ProviderInfo &p : known_providers()) {
      ...
      if (!ready && ImGui::IsItemHovered()) ImGui::SetTooltip("%s", tr("Add its key in Settings > AI services"));
    }
    ImGui::EndCombo();
  }
}
...
void image_window(App &a) {
  ...
  if (!ImGui::Begin(tr("Generate image"), &g_show_image)) { ImGui::End(); return; }
  static const char *const kinds[] = {"Image", "Tileable texture", "360 skydome"};
  int k = g_kind == JOB_TEXTURE ? 1 : g_kind == JOB_SKYDOME ? 2 : 0;
  ImGui::SetNextItemWidth(220);
  if (ImGui::Combo(tr("What"), &k, tr_combo(kinds, 3).c_str())) g_kind = k == 1 ? JOB_TEXTURE : k == 2 ? JOB_SKYDOME : JOB_IMAGE;
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", tr("A texture is asked for as a seamless top-down tile; a skydome as a 2:1 equirectangular panorama."));
  provider_combo(tr("Provider"), g_image_provider, "image", "image");
  ImGui::InputTextMultiline("##prompt", g_prompt, sizeof g_prompt, ImVec2(-1, 70));
  if (g_prompt[0] == 0) {
    ImGui::SameLine(12);
    ImGui::TextDisabled("%s", tr("describe it: wet mossy granite, fine cracks..."));
  }
  ImGui::SetNextItemWidth(-1);
  ImGui::InputTextWithHint("##neg", tr("avoid (optional)"), g_negative, sizeof g_negative);
  const char *sizes[] = {"512", "1024", "2048"};
  ImGui::SetNextItemWidth(100);
  ImGui::Combo(tr("Size"), &g_size, sizes, 3);
  ImGui::SameLine();
  ImGui::SetNextItemWidth(120);
  ImGui::InputInt(tr("Seed (0 random)"), &g_seed);
  if (g_image_provider == "comfyui") {
    ImGui::SetNextItemWidth(-1);
    ImGui::InputTextWithHint("##wf", tr("workflow (bundled text_to_image, or a name in your ComfyUI workflows folder)"),
                             g_workflow, sizeof g_workflow);
  }
  ImGui::Separator();
  if (g_kind == JOB_TEXTURE) {
    studio::Checkbox(tr("Use on the material open in the studio"), &g_apply_material);
    ...
    studio::Checkbox(tr("Set as the sky when done"), &g_apply_sky);
  }
  ImGui::BeginDisabled(g_prompt[0] == 0);
  if (ImGui::Button(tr("Generate"), ImVec2(140, 0))) {
  ...
  ImGui::EndDisabled();
  ImGui::SameLine();
  if (ImGui::Button(tr("Jobs..."))) g_show_jobs = true;
  ImGui::End();
}
...
void model_window(App &a) {
  ...
  if (!ImGui::Begin(tr("Generate 3D model"), &g_show_model)) { ImGui::End(); return; }
  provider_combo(tr("Provider"), g_model_provider, "3d", "3d");
  ImGui::InputTextMultiline("##mprompt", g_model_prompt, sizeof g_model_prompt, ImVec2(-1, 60));
  if (g_model_prompt[0] == 0) { ImGui::SameLine(12); ImGui::TextDisabled("%s", tr("a weathered granite boulder...")); }
  ImGui::SetNextItemWidth(-90);
  ImGui::InputTextWithHint("##ref", tr("reference picture (image-to-3D, optional; Hitem3D needs one)"), g_ref_image, sizeof g_ref_image);
  ImGui::SameLine();
  ...
  studio::Checkbox(tr("Import into the scene when done"), &g_import_model);
  ...
  ImGui::EndDisabled();
  ImGui::TextDisabled(tr("Results land in %s and in the asset index."), config_output_dir("models").c_str());
  ImGui::End();
}
...
void jobs_window(App &a) {
  ...
  if (!ImGui::Begin(tr("AI jobs"), &g_show_jobs)) { ImGui::End(); return; }
  if (ImGui::SmallButton(tr("Clear finished"))) ai_jobs_clear_finished();
  if (ImGui::BeginTable("##jobs", 5, ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingStretchProp)) {
    ImGui::TableSetupColumn("", 0, 0.5f);
    ImGui::TableSetupColumn(tr("Job"), 0, 2.2f);
    ImGui::TableSetupColumn(tr("State"), 0, 0.8f);
    ImGui::TableSetupColumn(tr("Progress"), 0, 1.2f);
    ...
    for (auto &j : ai_jobs()) {
      ...
      else ImGui::TextDisabled("%s", tr(ai_job_kind_name(j->kind)));
      ImGui::TableNextColumn();
      ImGui::TextWrapped("%s: %s", tr(ai_job_kind_name(j->kind)), j->prompt.substr(0, 80).c_str());
      ImGui::TextDisabled("%s", j->provider.c_str());
      ImGui::TableNextColumn();
      ImGui::TextUnformatted(tr(ai_job_state_name(s)));
      ImGui::TableNextColumn();
      ...
      if (s == JOB_RUNNING || s == JOB_QUEUED) {
        if (ImGui::SmallButton(tr("Cancel"))) ai_job_cancel(j->id);
      } else if (s == JOB_DONE && !result.empty()) {
        if (j->kind == JOB_MODEL) {
          if (ImGui::SmallButton(tr("Import"))) { j->apply.import_object = true; j->serviced = false; }
        } else if (j->kind == JOB_SKYDOME) {
          if (ImGui::SmallButton(tr("Use as sky"))) { j->apply.as_skydome = true; j->serviced = false; }
        } else if (j->kind != JOB_TEXT) {
          if (ImGui::SmallButton(tr("Use on material"))) {
...
void ai_tool_buttons() {
  if (ImGui::SmallButton(tr("AI model"))) g_show_model = true;
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", tr("Generate a 3D model from a prompt or a picture (Meshy, Tripo, Hitem3D)."));
  ImGui::SameLine();
  if (ImGui::SmallButton(tr("AI image"))) { g_kind = JOB_IMAGE; g_show_image = true; }
  if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", tr("Generate an image, a tileable texture or a 360 skydome."));
}
