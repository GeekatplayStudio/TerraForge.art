...
// The ground level the surround is drawn about (render_settings.hpp): one
// definition, so a population placed on that ground and the ground itself
// cannot disagree.
float renderer_ground_base() {
  extern bool g_terrain_base_set;
  return (g_terrain_base_set ? g_terrain_base : g_terrain_mean) *
