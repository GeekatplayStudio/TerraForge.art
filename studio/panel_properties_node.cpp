...
bool draw_attribute(gpx::Attribute &at) {
  ...
  switch (at.type) {
    ...
    case gpx::AttrType::Text: {
      // A text attribute that names a scene object gets the scene's objects
      // to choose from. A list you can see beats a name you have to remember,
      // and it is the only way to know what is available.
      if (at.object_ref) {
        changed = object_ref_combo(at);
        break;
      }
      char buf[512];
      std::strncpy(buf, at.s.c_str(), sizeof buf - 1);
