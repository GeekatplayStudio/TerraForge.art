...
void draw_statusbar(App &a) {
  ...
  add(tr("Frames shown per second in the main window (capped by vsync and the viewport rate),\n"
      "and how many the work would allow if nothing capped it."),
      slow ? warn : ok, tr("FPS %.0f (%.0f possible)"), s.fps, s.potential_fps);
  add(tr("Where a frame goes: interface, the 3D views, node previews, API, GPU uploads.\n"
      "CPU work per frame excludes the time the loop sleeps to the frame rate."),
      dim, tr("frame %.1f ms  ui %.1f  views %.1f  gpu %.1f"), s.work_ms, s.ui_ms, s.views_ms, s.gpu_ms);
  add(tr("The last graph evaluation, and how many terrain patches survived culling this frame."), dim,
      tr("eval %.0f ms  %d nodes  %d patches"), s.eval_ms, s.nodes, s.patches);
  add(tr("This process's working set (peak), and the memory the system still has free."),
      s.system_free_mb > 0 && s.system_free_mb < 1024 ? warn : dim, tr("RAM %s (peak %s)  free %s / %s"),
  ...
  add(tr("This process's share of every core."), s.cpu_pct > 80 ? warn : dim, tr("CPU %.0f%% of %d"), s.cpu_pct, s.cpu_cores);
  if (s.vram_total_mb > 0) {
    char v1[32], v2[32];
    add(tr("Video memory in use / total, as the driver reports it."), s.vram_used_mb > s.vram_total_mb * 0.9 ? warn : dim,
        tr("VRAM %s / %s"), human_mb(s.vram_used_mb, v1, sizeof v1), human_mb(s.vram_total_mb, v2, sizeof v2));
  } else {
    add(tr("The driver does not report video memory over OpenGL (NVIDIA and AMD do)."), dim, "%s", tr("VRAM n/a"));
  }
  add(s.gpu_name.c_str(), dim, "%s", s.gpu_name.size() > 28 ? (s.gpu_name.substr(0, 27) + "~").c_str() : s.gpu_name.c_str());
  add(pc.governor ? tr("The performance governor: when the work would drop the main view below the\n"
                  ...
                  : tr("The governor is off (Settings > General)."),
      s.governor_level ? warn : ok, "%s", pc.governor ? (s.governor_level ? s.governor_note.c_str() : tr("governor: full")) : tr("governor off"));
  float total = 0.f;
